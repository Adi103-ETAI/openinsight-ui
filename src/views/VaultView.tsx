        ...
        {filteredItems.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredItems.map((item) => {
              ...
              const displayText = isExpanded ? item.chunkText : item.chunkText.slice(0, 150) + (truncated ? "…" : "");
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-3 pt-2 border-t border-border/50">
                    <span className="text-[10px] text-muted-foreground/60">Saved {formatDistanceToNow(item.savedAt, { addSuffix: true })}</span>
                    <span className="text-[10px] text-muted-foreground/60 font-mono">Relevance: {Math.round(item.score * 100)}%</span>
                  </div>
                </div>
              );
            })}
